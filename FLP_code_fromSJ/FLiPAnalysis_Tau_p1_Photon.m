function [ ] = FLiPAnalysis_Tau_p1_Photon(FLPdata_time, FLPdata_lifetimes, timebin,ch, filename) %timebin should be the same as slice time (i.e. ~0.85ns)

% calculate lifetime, photoncount, and p1 from FliP data.
% new data file in regular arrays
% tau_avg,tau_avgTrunc,tau_empTrunc,photoncount,p1,time
...
%         spc_adjustTauOffset(1); % update TauOffset
        if ~isfield(spc.fits{ch},'failedFit') || spc.fits{ch}.failedFit || ...
            ...
            tau_avg(i)=spc.fits{ch}.avgTau; % mean Tau calculated from fit
            tau_avgTrunc(i)=spc.fits{ch}.avgTauTrunc;
            tau_empTrunc(i)=spc.fits{ch}.EmpTauTrunc;
            chi2(i)=spc.fits{ch}.redchisq;
            beta2(i)=spc.fits{ch}.beta2;
            beta4(i)=spc.fits{ch}.beta4;
            beta5(i)=spc.fits{ch}.beta5;
            beta6(i)=spc.fits{ch}.beta6;
            backcorr(i)=spc.fits{ch}.backCorr;
            lifetime_histograms=[lifetime_histograms lifetimes];
...
backcorr=transpose(backcorr);

save(filename, 'time','photoncount', 'tau_avg','tau_avgTrunc','tau_empTrunc', 'p1','chi2','beta2','beta4','beta5','beta6','backcorr','lifetime_histograms');
