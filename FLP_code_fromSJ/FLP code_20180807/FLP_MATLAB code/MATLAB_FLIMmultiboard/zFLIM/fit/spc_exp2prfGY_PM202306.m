function y = spc_exp2prfGY_PM202306(par, x)
global spc;

TAC_offset_shift = 19;
x_channels = 1:1:spc.datainfo.adc_re;
...
pulse_int = 205; % Pingchuan 2023.05.04
...
prf1 = spc.fit.prf;
prf = interp1(x_channels, prf1, x_channels+deltapeak, 'linear');
prf(isnan(prf)) = 0;

lenx = length(x_channels);
...
y2 = zeros(pulse_int+TAC_offset_shift, 1);
y2 (TAC_offset_shift:pulse_int+TAC_offset_shift) = y1 (pulse_int+TAC_offset_shift+1:2*pulse_int+TAC_offset_shift+1); % modified by Pingchuan, 2023.06.14, 
y1 = y1(1:pulse_int+TAC_offset_shift);
