...
if (spc.switches.imagemode == 1 && timerGetActiveStatus('zFLIM')==1)
...
%     end
   ...
   try  % FIRST - select only the data with intensity>=LutLower
        ...
        if length(size(spc.imageMods{chan})) == 3
            ...
            % modified by Suk Joon lee 9/27/2016 for 'single mode'
            % in the case of 'single' mode, spc.imageMods{chan}=2D matrix
            index = repmat (index(:),[spc.size(1),1]);
            imageMod = index .*  spc.imageMods{chan};
        end
   catch
...
%     end
    ...
    if length(size(spc.imageMods{chan})) == 2 
        %when in 'single mode', scan-x=1 and scan-y=1 %modified by Suk Joon Lee 9/27/2016
        %imageMod is 1D matrix (time resolution x 1) containing photon counts
        spc.lifetimes{chan} = reshape(imageMod,1,spc.size(1));
    else
        %when in 'scan in' mode (with scanner)
        %imageMod is 3D matrix (time resolution x scan x-direction x scan y-direction) containing photon counts
        try
            ...
            %edited due to error 9/26/2016 - Suk Joon Lee
            if isfield(gui.spc.projects{chan},'mapRoi')
...
if(timerGetActiveStatus('zFLIM')==1) %% for FLIM purpose not FLP
