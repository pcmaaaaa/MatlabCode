...
for k=1:1;
    ...
    numPts=length(find(spc.lifetimes{1})>0);
   ...
   % time = length(index) * 12.5/256; %pulse width in ns
    ...
    disp([ 'base / peak: ',num2str((baseline(k)/(max(spc.lifetimes{k})-DarkCount(k)/numPts) ))]);
    display(['peak: ', num2str(max(spc.lifetimes{k})-DarkCount(k)/numPts), ' base: ', num2str(base(k)-DarkCount(k)/numPts)]);
