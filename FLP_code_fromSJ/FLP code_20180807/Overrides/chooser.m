...
% Last Modified by GUIDE v2.5 14-Nov-2016 10:46:20
...
% --- Executes on button press in flim.
function flim_Callback(hObject, eventdata, handles)
% hObject    handle to flim (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
    hideGUI('gh.chooser.figure1');
    global state
    initTimer(state.analysisMode, {'Imaging', 'LineMonitor', 'zFLIM'});
...
% handles    structure with handles and user data (see GUIDATA)
    hideGUI('gh.chooser.figure1');
    global state
    initTimer(state.analysisMode, {'zFLP'});