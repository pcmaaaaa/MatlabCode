...
%mousenamelist = {'SJ281','SJ282','SJ283','SJ284'};
mousenamelist = {'SJ281_blue LED','SJ281_lime LED','SJ282_blue LED','SJ282_lime LED','SJ283_blue LED','SJ283_lime LED','SJ284_blue LED','SJ284_lime LED'};
...
ch_name={'jRCaMP green','jRCaMP red'};
num_trial=10;
num_days=1;
    ...
    for d=1:length(TrialData(1).ch_name);
        ...
        for i=1:length(daylist)
            ...
            if(size(output,1)>0)
                ...
                output=selectTrialData(TrialData,'raw_dispense',d,[1],idx_start,idx_end);
                temp=mean(output,1);
                figure(2);
                plot(timestamp,temp(1:length(timestamp)),'color',cmap(i,:));
                title('raw signal vs. time(s): 0s = pellet dispensed');
                hold on;
                legend_mark(2,i)=1;
...
%                 xlswrite(filename,out,1);
            ...
            if(size(output,1)>0)
                ...
                figure(3);                
                plot(timestamp,temp(1:length(timestamp)),'color',cmap(i,:));
                title('df/f(%) vs. time(s): 0s = max amplitude');
                hold on;
                legend_mark(3,i)=1;
                
                output=selectTrialData(TrialData,'raw_dispense_max',d,[1],idx_start,idx_end);
                temp=mean(output,1);
                figure(4);
                plot(timestamp,temp(1:length(timestamp)),'color',cmap(i,:));
                title('raw signal vs. time(s): 0s = max amplitude');
                hold on;
                legend_mark(4,i)=1;
...
%                 xlswrite(filename,out,1);
            ...
            if(size(output,1)>0)
                ...
                output=selectTrialData(TrialData,'raw_receptacle',d,[1],idx_start,idx_end);
                temp=mean(output,1);
                figure(6);
                plot(timestamp,temp(1:length(timestamp)),'color',cmap(i,:));
...
%                 out=timestamp';
%                 out=[out,output'];
%                 filename=[excel_filename,mousenamelist{mouse},'_',filetitle{4},'.xlsx'];
%                 xlswrite(filename,out,1);
            end
