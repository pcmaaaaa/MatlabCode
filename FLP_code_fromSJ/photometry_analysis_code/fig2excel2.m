function [xls] = fig2excel(xls, figlist, mouse, channel, TrialData, filename)
...
% xls.make_excel = 1;
% xls.overwrite = 1;
% xls = fig2excel2(xls,figurelist,mousenamelist(mouse), TrialData(1).ch_name(d), TrialData, 'results.xlsx');
...
%parameters
if ~isfile(filename) || (xls.overwrite == 1 && ~isfield(xls, 'firstrun')) 
    xls.firstrun = 0;
...
for i = 1:length(figlist)
    ...
    %saving figure
    print(['-f' num2str(figlist(i))], [pwd '\figuredir\' figname '.png'],'-dpng', '-r300')
...
xls.refrow = skip * xls.space_y*18 + xls.refrow; 
SaveAs(Workbook,[pwd '\' filename])
end