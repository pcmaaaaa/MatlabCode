function [dtau,photoncount,time] = align_FLIM_signal_downsample(FLPdata_time, FLPdata_lifetimes, eventtime,duration,baseline_duration,timebin,ch,histogram_bin)
...
baseline_tau = spc_calculateAvgTau_downsample(ch,histogram_bin);
...
for i=idx_start:1:idx_end
    if counter2==0 || (FLPdata_time(i,ch) > ceil(bin_start) && FLPdata_time(i,ch) <= ceil(bin_start + timebin) && counter2<timebin)
        ...
        counter = counter + 1;
        spc.lifetimes{ch}=lifetimes;
        tau(counter)=spc_calculateAvgTau_downsample(ch,histogram_bin);
        ...
        if(counter>2 && (photoncount(counter)-photoncount(counter-1))/photoncount(counter-1)>0.05)
            tau(counter)=tau(counter-1);
            photoncount(counter)=photoncount(counter-1);
            spc.lifetimes{ch}=lifetimes;
            %spc_drawLifetime(ch);
            display(i);
            display('');
        elseif(counter>2 && abs(tau(counter)-tau(counter-1))>0.3)
            tau(counter)=tau(counter-1);
            photoncount(counter)=photoncount(counter-1);
            spc.lifetimes{ch}=lifetimes;
            %spc_drawLifetime(ch);
