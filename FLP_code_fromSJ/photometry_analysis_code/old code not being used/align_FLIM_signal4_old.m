function [dtau,photoncount,time] = align_FLIM_signal4(FLPdata_time, FLPdata_lifetimes, eventtime,duration,baseline_duration,timebin,ch,baseline_tau_duration)
...
%     baseline_taus=[baseline_taus,spc_calculateAvgTau3(ch)];
% end
baseline_tau = mean(baseline_taus);
% spc.lifetimes{ch}=lifetimes;
% baseline_tau = spc_calculateAvgTau3(ch);
        ...
        if FLPdata_time(i,ch)-FLPdata_time(i-1,ch) > 1.5*timebin
            ...
            while(1)
                if counter>1
                    ...
                    %tau(counter)=(spc_calculateAvgTau3(ch)+tau(counter-1))/2;
                else
                    tau(counter)=spc_calculateAvgTau3(ch);
                end
                photoncount(counter)=sum(lifetimes);
