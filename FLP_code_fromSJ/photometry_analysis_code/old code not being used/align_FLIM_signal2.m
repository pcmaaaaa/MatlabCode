function [dtau,photoncount,time] = align_FLIM_signal2(FLPdata_time, FLPdata_lifetimes, eventtime,duration,baseline_duration,timebin,ch,baseline_tau_duration)
%   calculated delta lifetime in a defined period
% old data file in cells
...
baseline_tau = spc_calculateAvgTau(ch);
...
for i=idx_start:1:idx_end
    if counter2==0 || (FLPdata_time(i,ch) > ceil(bin_start) && FLPdata_time(i,ch) <= ceil(bin_start + timebin) && counter2<timebin)
        ...
        counter = counter + 1;
        spc.lifetimes{ch}=lifetimes;
        tau(counter)=spc_calculateAvgTau(ch);
