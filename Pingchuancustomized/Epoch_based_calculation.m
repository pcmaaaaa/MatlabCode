...
for i_cycle = 1:length(CyclePositions)
    ...
    for i = 1:size(intensity, 2)
        ...
        epoch_response_intensity(j,i) = mean(intensity((drug_epoch_start_acq(drug_epoch_index(j+1))-epoch_acq_num):(drug_epoch_start_acq(drug_epoch_index(j+1))-1),i),'omitnan');
        scatter((drug_epoch_start_acq(drug_epoch_index(j+1))-epoch_acq_num):(drug_epoch_start_acq(drug_epoch_index(j+1))-1), intensity((drug_epoch_start_acq(drug_epoch_index(j+1))-epoch_acq_num):(drug_epoch_start_acq(drug_epoch_index(j+1))-1),i),25)
        hold on
        end
    end
...
%     ylim([3.8 4.15])
    ...
    for i = 1:size(intensity, 2)
        for j = 1:length(drug_epochs)-1
        epoch_response_lft(j,i) = mean(lft((drug_epoch_start_acq(drug_epoch_index(j+1))-epoch_acq_num):(drug_epoch_start_acq(drug_epoch_index(j+1))-1),i),'omitnan');
        scatter((drug_epoch_start_acq(drug_epoch_index(j+1))-epoch_acq_num):(drug_epoch_start_acq(drug_epoch_index(j+1))-1), lft((drug_epoch_start_acq(drug_epoch_index(j+1))-epoch_acq_num):(drug_epoch_start_acq(drug_epoch_index(j+1))-1),i),25)
        hold on
        end
    end
    hold off

    saveas(gcf, [summary_path, stateYao.baseName, '_', 'cycle ', num2str(CyclePositions(i_cycle)),'.fig'])
    saveas(gcf, [summary_path, stateYao.baseName, '_', 'cycle ', num2str(CyclePositions(i_cycle)),'.png'])
