...
for m=state.spc.acq.modulesInUse  % gy multiboard 201202
    ...
    %state.spc.acq.SPCdata{m+1}.mode = 2; %manually setting aquisition mode to Scan IN
    
    %calculating collect_time (in sec) from n# of frames, lines per frame
    %and ms per line defined by imaging control configuration and cycle
    %definition
    state.spc.acq.SPCdata{m+1}.collect_time = state.acq.numberOfFrames*state.acq.linesPerFrame*state.acq.msPerLine*1e-3;
    ...
    state.spc.acq.SPCdata{m+1}.pixel_time = 100/(state.acq.inputRate/state.acq.binFactor);
