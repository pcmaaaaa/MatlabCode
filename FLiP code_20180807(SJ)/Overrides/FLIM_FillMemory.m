...
switch state.spc.acq.SPCdata{m+1}.mode
    case {0,1}
        if state.FLP.sequencer==1
            for i=0:state.spc.acq.SPCMemConfig{m+1}.maxpage-1
                out1=calllib('spcm32','SPC_fill_memory',m,0,i,0); %fill all pages in the current memory bank
                if out1 ~= 0
