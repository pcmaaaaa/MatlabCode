...
% SJ 201611 Modified to make sure spc_fits for all active channels are initialized
%properly
if exist('spc_fits','var') == 1
    for fc=FLIMchannels
        if(length(spc_fits)>=fc && isempty(spc_fits{fc})==0) %if the previous spc_backup.mat has spc fit data for the same channel
            spc.fits{fc} = spc_fits{fc};
            spc_translateFitsToFit(fc);
        else
            % "default values"
        ...
        if(spc.fits{fc}.fitend==0) %if the previous spc_backupmat had an empty data (zero)
            % "default values"
            spc.fit.beta0(1:6)=[1000 20 1000 10 5 20];
            spc.fit.fixtau1=1;
            spc.fit.fixtau2=1;
...
if exist('spc_filename') == 2 %modified by Suk Joon Lee 9/27/2016 due to error from "exist(spc_filename) -> exist('spc_filename')
