...
%    sseold = r'*r;
   sseold = sum( (double(y)-yfit).^2 ./ max(yfit,1) );  % gy 20120718 modified 201305 to eliminate negative value problem
   if gydebug
...
%    sse = rnew'*rnew;
   sse = sum( (double(y)-yfitnew).^2 ./ max(yfitnew,1) );  % gy 20120718 modified 201305 to eliminate negative value problem
   if gydebug
