...
for m=state.spc.acq.modulesInUse
    ...
    for jj=1:size(framesChans,1)
        frame=framesChans(jj,1); % first column is the frame (c1 or c2 of each board)
        ...
        if frame>frames_per_page || frame>scan_rout_x
            ...
            if state.acq.numberOfZSlices>1
                ...
                spc.imageMods{chan}=imageF;
                spc.imageModSlices={};
                spc.projects{chan}  = reshape(sum(spc.imageMods{chan}, 1), scan_size_y, scan_size_x); %total photon counts in a pixel
            end
            
            % calculate the total number of photons for each time bin, added
            % across all pixels
