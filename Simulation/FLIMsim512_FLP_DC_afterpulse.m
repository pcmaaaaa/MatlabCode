function FLIMsim512_FLP_DC_afterpulse = FLIMsim512_FLP_DC_afterpulse(DC, afterpulse, SaveName)
...
Sample=DarkCounts; % Sample = signal+dark counts.  A vector of lifetimes.
% figure;
% display(length(Sample));
% hist(Sample, 256);
...
Afterpulses=zeros(1, afterpulse); % The number of afterpulses is 1% of total photon counts (signal+dark counts)
Afterpulses=xsim(randi(length(xsim), 1, afterpulse)); % Afterpulses are randomly and evenly distributed in lifetime bins.  Note this is an assumption; in reality, they occur at distinct time points (e.g. 3) after photon signal.
...
%     b=rand();
%     a=find(histc(b,[0;cumsum(Gaussian_y(:))]))-1;
% %     a=find(histc(rand(),[cumsum(prf(:))]))-37;
%     
% %     display(rng)
% %     display(i)
% %     display(SampleC(i))
% %     display(Sample(i))
% %     display(a)
%     SampleC(i)=Sample(i)+a*(25/256); % Randomly draw a distribution based on the probability specified in the prf.
% %     if SampleC(i)>25
% %         SampleC(i)=SampleC(i)-25; % Wrap around the distribution.
% %     else if SampleC(i)<0
% %         SampleC(i)=SampleC(i)+25; %Wrap around the distribution.
% %         end
% %     end
...
% for i=1:AF
%     Autofluorescence(i)=find(histc(rand(),[cumsum(autofluo(:));1]))*(25/256); % Draw a lifetime for autofluorescence based on the probability specified in the measured autofluorescence file.
% end
...
[n, xout]=hist(Sample, xsim);
% [n2, xout2]=hist(SampleC, xsim);
...
save(SaveName, 'n'); % n gives the photon count per bin; xout gives the lifetime position of each bin.
end
