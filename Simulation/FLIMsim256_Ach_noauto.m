function FLIMsim256_ACh_noauto = FLIMsim256_ACh_noauto(SS, DC, AF, prf_peak, PopulationName, PrfName, AutofluorescenceName)
% global n;
...
for i=1:length(Sample)
    a=find(histc(rand(),[cumsum(prf(:))]))-1;
...
% load(AutofluorescenceName); %Load from a probability distribution of autofluorescence in Autofluorescence_2FLIM001, measured at 30um below the surface of an 
...
save('FLIMSimulation_pm', 'n', 'xout'); % n gives the photon count per bin; xout gives the lifetime position of each bin.
end
